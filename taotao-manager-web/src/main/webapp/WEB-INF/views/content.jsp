...
$(function(){
	...
	tree.tree({
		...
		onClick : function(node){
			//判断节点是否是叶子节点，如果是，执行以下逻辑
			if(tree.tree("isLeaf",node.target)){
				//datagrid执行reload，就是重载
...
var contentListToolbar = [{
    ...
    handler:function(){
    	...
    	//if(node):判断node不为空或者不是未定义，以下代码是！加了非的
    	//tree("isLeaf",node.target):判断节点是必须是叶子节点，以下代码是！加了非的
    	if(!node || !$("#contentCategoryTree").tree("isLeaf",node.target)){
    		$.messager.alert('提示','新增内容必须选择一个内容分类!');
    		return ;
    	}
    	//创建窗口，窗口的内容请求地址是/rest/page/content-add"
    	createWindow({
